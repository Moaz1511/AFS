    ...
    for para in paragraphs:
        ...
        if state == 0:
            ...
            if opt_re.match(text):
                ...
                cur['question_lines_meta'].append((text, style, ommls))
                continue
        if state == 2:
            m = opt_re.match(text)
            if m:
                cur['options'][m.group(1)] = (m.group(2), ommls)
                continue
                ...
                if cur['options']:
                    ...
                    cur['options'][last_opt] = (val + ' ' + text, oxmls)
    return mcqs
...
def main():
    ...
    out = "Reformatted_MCQ_Sheet check.docx"
    doc = Document(src)
