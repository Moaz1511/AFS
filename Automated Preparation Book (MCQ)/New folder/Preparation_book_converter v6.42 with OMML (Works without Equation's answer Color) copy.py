...
def format_mcq(mcq, doc):
    ...
    # Add question (removing duplicate serial if needed)
    ...
    # Add reference if any (in same line)
    ...
    for k in labels:
        ...
        if v:
            opt_text, src_para = v
            if src_para:
                # If option contains OMML, get text and ommls
                ...
                for node in tree.iterchildren():
                    if node.tag.endswith('r'):
                        ...
                        for t in texts:
                            txt = t.text or ""
                            # Remove label from original, if present
                            if txt.strip().startswith(f"{k}."):
                                txt = txt.strip()[len(f"{k}."):].lstrip()
                            textval += txt
                    elif node.tag.endswith('oMath') or node.tag.endswith('oMathPara'):
                        ommls.append(etree.tostring(node, encoding='unicode'))
                opt_text = textval.strip()
            else:
                ommls = []
    ...
    # Decide row format: two options per row or single column if too long
    LONG_LIMIT = 18  # Adjust for your visual preference
    if max_len > LONG_LIMIT:
        # One option per line (long text/equation)
