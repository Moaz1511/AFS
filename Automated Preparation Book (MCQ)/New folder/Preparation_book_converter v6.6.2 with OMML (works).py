...
SHORT_OPTION_CHAR_LIMIT = 5    # All 4 ≤ this => 1 line
MEDIUM_OPTION_CHAR_LIMIT = 15  # All 4 ≤ this => 2 lines
LONG_OPTION_CHAR_LIMIT = 20    # If any > this => 4 lines

OMML_WEIGHT_SHORT = 5    # OMML length ≤150 chars (XML) gets this weight
OMML_WEIGHT_MEDIUM = 12  # OMML length ≤350 chars
OMML_WEIGHT_LONG = 1000    # OMML length >350 chars
...
def _omml_visible_length(omml_xml):
    """
    Returns the 'visible' character count of the OMML math equation.
    This is a better proxy than XML length!
    """
    try:
        tree = etree.fromstring(omml_xml)
        # OMML text usually in all <m:t> tags (math text)
        ...
        return 12  # fallback: medium

def _option_effective_length(para, label):
    """
    Returns visual 'length' of option: text + OMML (weighted).
    """
    total = 0
...
def get_option_length_class(options_dict):
    """
    - 'oneline' if ALL 4 options ≤ SHORT_OPTION_CHAR_LIMIT
    - 'twoline' if ALL 4 options ≤ MEDIUM_OPTION_CHAR_LIMIT
    - otherwise 'fourline'
    """
    lens = []
    for label in OPTION_LABELS:
        ...
        if not opt_tuple:
            return "fourline"  # Missing option, go safest
        para, _ = opt_tuple
...
def main():
    ...
    # Tab stops (edit as needed for visual alignment)
