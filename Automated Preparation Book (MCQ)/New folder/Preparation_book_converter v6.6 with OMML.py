...
LONG_OPTION_CHAR_LIMIT = 50    # <= 4 lines (1 per line)  # Character length to trigger single-column layout
...
def format_mcq(mcq, doc, tab_stops_oneline, tab_stops_twoline):
    ...
    if option_layout == "oneline":
        # Four options in one line with 3 tabs (for 4 columns)
        ...
        # Two lines, two options per line, with 1 tab (for 2 columns)
        for row in [(0, 1), (2, 3)]:
            option_para = doc.add_paragraph()
            option_para.paragraph_format.tab_stops.clear_all()
        ...
        # Four lines, one option per line
        for idx in range(4):
            label = OPTION_LABELS[idx]
            opt_tuple = mcq['options_meta'].get(label)
            if not opt_tuple:
