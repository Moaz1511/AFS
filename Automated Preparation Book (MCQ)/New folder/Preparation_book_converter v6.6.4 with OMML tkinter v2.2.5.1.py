...
def is_extra_heading(text):
    keywords = ["∆ নিচের তথ্যের আলোকে", "উদ্দীপক", "নিচের", "তথ্যের ভিত্তিতে", "তথ্যের আলোকে", "প্রশ্নের উত্তর দাও", "নিচের তথ্যের আলোকে", "নং প্রশ্নের উত্তর দাও:"]
...
# --- NEW: Advanced LaTeX Equation Converter for Data Export ---
MML_NS_EXPORT = 'http://schemas.openxmlformats.org/officeDocument/2006/math'
...
def _parse_omml_to_latex_recursive(node):
    ...
    if tag == 't':
        ...
        num = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}num'))
        den = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}den'))
        return f"\\frac{{{num.strip()}}}{{{den.strip()}}}"
    elif tag == 'sSup':
        base = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}e'))
        sup = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}sup'))
        return f"{{{base.strip()}}}^{{{sup.strip()}}}"
    elif tag == 'sSub':
        base = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}e'))
        sub = _parse_omml_to_latex_recursive(node.find(f'./{{{MML_NS_EXPORT}}}sub'))
...
def omml_to_latex(omml_xml):
    """Converts OMML to a pure LaTeX string for data export."""
    try:
        tree = etree.fromstring(omml_xml)
        math_node = tree.find(f'.//{{{MML_NS_EXPORT}}}oMath') or tree.find(f'.//{{{MML_NS_EXPORT}}}oMathPara')
        if math_node is None:
...
def get_para_content_for_export(para):
    """Helper to get paragraph text with equations converted to pure LaTeX."""
    full_str = ""
    parts = split_text_and_omml(para)
    for ctype, cvalue in parts:
        if ctype == "text":
            full_str += cvalue
        elif ctype == "omml":
            full_str += omml_to_latex(cvalue)
    return full_str.strip()

def mcqs_to_rows(mcqs):
    """
    Converts MCQ list to rows for export with correct Type, Reference, Answer Label,
    Answer Text, Explanation, and LaTeX, without modifying the extract_mcqs function.
    """
    rows = []
    for mcq in mcqs:
        # --- 1. Process Question Block for Type and Reference ---
        all_question_paras = [p for p, _ in mcq['question_meta']]
        full_question_text_for_parsing = " ".join([p.text for p in all_question_paras])

        references = re.findall(r'(\[.*?\])', full_question_text_for_parsing)
        reference_text = " ".join(references).strip()
        
        q_type = 'সাধারণ'
        type_keywords = ['জ্ঞানমূলক', 'অনুধাবনমূলক', 'প্রয়োগমূলক', 'উচ্চতর দক্ষতামূলক']
        
        found_type_in_ref = None
        for keyword in type_keywords:
            keyword_tag = f'[{keyword}]'
            if keyword_tag in reference_text:
                found_type_in_ref = keyword
                reference_text = reference_text.replace(keyword_tag, '').strip()
                break
        
        if found_type_in_ref:
            q_type = found_type_in_ref
        else:
            is_bahupadi = any(p.text.strip().startswith(r) for p in all_question_paras for r in ROMAN_NUMERALS)
            has_stem = len(all_question_paras) > 1 or 'উদ্দীপক' in full_question_text_for_parsing
            if is_bahupadi:
                q_type = 'বহুপদী সমাপ্তিসূচক'
            elif has_stem:
                q_type = 'উদ্দীপকভিত্তিক'

        # --- 2. Build Final Content with LaTeX ---
        question_text_latex = " ".join(get_para_content_for_export(p) for p in all_question_paras)
        
        for ref_tag in references:
            question_text_latex = question_text_latex.replace(ref_tag, '')
        
        question_final = re.sub(r'^\s*[\(]?\s*[\d০-৯]+[\.\)\।]?\s*', '', question_text_latex).strip()
        
        # Process Options with robust cleaning
        options = {}
        for label in OPTION_LABELS_BN:
            opt_tuple = mcq['options_meta'].get(label)
            if opt_tuple:
                opt_para = opt_tuple[0]
                opt_text_raw = get_para_content_for_export(opt_para)
                # FIX: Use a robust regex to strip all label variations (e.g., "ক.", "(ক)")
                options[label] = re.sub(fr'^\s*[\(\[]?{label}[\.\)\]]?\s*', '', opt_text_raw).strip()
            else:
                options[label] = ""
        
        # Process Answer Text
        answer_text = ""
        if mcq.get('answer_meta'):
            ans_para = mcq['answer_meta'][0]
            answer_text_raw = get_para_content_for_export(ans_para)
            # FIX: Use keyword argument `flags=` to remove DeprecationWarning
            answer_text = re.sub(r'^উত্তর[:：ঃ]?\s*[\(\[]?\s*[কখগঘa-dA-D]?[\.\)\]\।:ঃ]?\s*', '', answer_text_raw, flags=re.IGNORECASE).strip()

        # Process Explanation Text
        explanation_text = ""
        if mcq.get('explanation_meta'):
            exp_para = mcq['explanation_meta'][0]
            explanation_text_raw = get_para_content_for_export(exp_para)
            # FIX: Use keyword argument `flags=` to remove DeprecationWarning
            explanation_text = re.sub(r'^ব্যাখ্যা[:：ঃ]?\s*', '', explanation_text_raw, flags=re.IGNORECASE).strip()

        # --- 4. Find Missing Answer Label by matching cleaned text ---
        answer_label = mcq.get('answer_label')
        if not answer_label and answer_text:
            for label, option_text in options.items():
                if option_text and answer_text == option_text:
                    answer_label = label
                    break
        
        # --- 5. Assemble the Final Data Row ---
        row = {
            'Serial': mcq['serial'],
            'Type': q_type,
