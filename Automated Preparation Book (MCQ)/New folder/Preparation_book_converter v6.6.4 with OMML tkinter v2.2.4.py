...
def omml_to_latex_text(omml_xml):
    try:
        tree = etree.fromstring(omml_xml)
        # Handle powers (superscripts)
        out = ""
        for node in tree.iter():
            tag = etree.QName(node.tag).localname
            if tag == "sSup":  # Superscript structure
                # Base
                base = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}e//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                # Superscript
                sup = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}sup//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                out += f"{base}^{sup}"
            elif tag == "sSub":  # Subscript structure
                base = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}e//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                sub = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}sub//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                out += f"{base}_{sub}"
            elif tag == "f":  # Fraction
                num = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}num//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                den = "".join([t.text or "" for t in node.findall('.//{http://schemas.openxmlformats.org/officeDocument/2006/math}den//{http://schemas.openxmlformats.org/officeDocument/2006/math}t')])
                out += f"({num})/({den})"
            elif tag == "t":
                out += node.text or ""
            elif tag == "rad":
...
def mcqs_to_rows(mcqs):
    """
    Convert MCQ list to rows of dicts for DataFrame/CSV/Excel.
    OMML is converted to Unicode latex-like math text.
    """
    rows = []
    for mcq in mcqs:
        # --- Question ---
        q_parts = []
        for p, _ in mcq['question_meta']:
            parts = split_text_and_omml(p)
            for typ, val in parts:
                if typ == "text":
                    q_parts.append(val)
                elif typ == "omml":
                    q_parts.append(omml_to_latex_text(val))
        # Remove serial from question (first Bangla/English number and period)
        question = re.sub(r'^\s*[\(]?\s*[\d০-৯]+[\.\)\।]?\s*', '', " ".join(q_parts)).strip()
        ...
        for label in OPTION_LABELS_BN:
            ...
            if opt_tuple:
                parts = split_text_and_omml(opt_tuple[0], strip_label_prefix=label)
                opt_text = ""
                for i, (typ, val) in enumerate(parts):
                    # Add space between text/omml as in Word
                    if i > 0 and not (parts[i-1][1].endswith(' ') or val.startswith(' ')):
                        opt_text += " "
                    if typ == "text":
                        opt_text += val
                    elif typ == "omml":
                        opt_text += omml_to_latex_text(val)
        ...
        elif mcq.get('answer_meta'):
            para, raw_text = mcq['answer_meta']
            parts = split_text_and_omml(para, strip_label_prefix="উত্তর")
            ans_combined = ""
            for i, (typ, val) in enumerate(parts):
                if i > 0 and not (parts[i-1][1].endswith(' ') or val.startswith(' ')):
                    ans_combined += " "
                if typ == "text":
                    ...
                    ans_combined += omml_to_latex_text(val)
            answer_text = ans_combined.strip()
        if not answer_label:
            for label, opt in options.items():
                if answer_text and answer_text.strip() == opt.strip():
                    answer_label = label
                    break
        ...
        if mcq.get('explanation_meta'):
            ...
            for i, (typ, val) in enumerate(parts):
                ...
                if typ == "text":
                    ...
                    exp_text += omml_to_latex_text(val)
