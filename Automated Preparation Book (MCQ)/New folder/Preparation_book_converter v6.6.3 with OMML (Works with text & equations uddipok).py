    ...
    for i, para in enumerate(paragraphs):
        ...
        for line in lines:
            ...
            if state == "find_question":
                ...
                # ---> Only update if cur is not None!
                if cur is not None:
...
def main():
    ...
    # Now, write to output docx: keep other text, insert formatted MCQ blocks, but do NOT duplicate answers!
    i = 0
    while i < len(doc.paragraphs):
        if i in para_to_mcq:
            mcq_idx = para_to_mcq[i]
            # Only write the MCQ at its first paragraph
            ...
            # skip all paragraphs belonging to this MCQ
    ...
    # To avoid duplicate MCQ output: map all paragraph indices used in MCQs
    mcq_para_indices = set()
    for mcq in mcqs:
        mcq_para_indices.update(mcq['all_paras'])
    i = 0
    while i < len(doc.paragraphs):
        if i in para_to_mcq and i not in mcq_para_indices:
            # Already handled as part of MCQ
            i += 1
            continue
        if i in para_to_mcq:
            mcq_idx = para_to_mcq[i]
            # Print this MCQ if first para of its group
            mcq = mcqs[mcq_idx]
            if mcq['all_paras'][0] == i:
                format_mcq(mcq, outdoc, tab_stops_oneline, tab_stops_twoline)
            # Skip all paras for this MCQ group
            i = mcq['all_paras'][-1] + 1
            continue
        # Print any other non-MCQ text (উদ্দীপক, section headers, etc.)
        para = doc.paragraphs[i]
        out_p = outdoc.add_paragraph()
        # Copy text with Bangla font
        run = out_p.add_run(para.text)
        for run in out_p.runs:
            set_bangla_font(run)
        # Also try to copy paragraph alignment/format if needed
        out_p.paragraph_format.space_after = para.paragraph_format.space_after
        out_p.paragraph_format.space_before = para.paragraph_format.space_before
        i += 1
