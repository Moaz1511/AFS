...
def parse_option(line):
    """
    Matches ক. খ) (গ), গ. (ঘ) A. (a) (A) etc.
    Returns (label, text) with all trailing punctuations/brackets removed from label.
    """
    m = re.match(r'^\s*[\(\[]?([কখগঘa-dA-D])[\.\)\]\।]?\s*(.*)', line)
    if m:
...
def parse_answer(line):
    """
    Matches: উত্তর: ঘ. ...  উত্তরঃ (ঘ) ... উত্তর: স্বাভাবিক সংখ্যা
    Always strips trailing dot/paren and spaces from label.
    """
    # Matches answer with label (e.g., ঘ, a, etc.)
    m = re.match(r'^উত্তর[:：ঃ]?\s*[\(\[]?([কখগঘa-dA-D])[\.\)\]\।]?\s*(.*)', line)
    if m:
        label = m.group(1)
        rest = m.group(2).strip()
        return (label, rest)
    # Matches answer text only (no label)
...
def parse_explanation(line):
    # ব্যাখ্যা: or ব্যাখ্যাঃ or ব্যাখ্য়া:
    ...
    for i, para in enumerate(paragraphs):
        ...
        for line in lines:
            ...
            if state == "find_question":
                ...
                cur['all_paras'].append(i)
        # If answer is found, but explanation comes next paragraph, attach it!
...
def format_mcq(mcq, doc, tab_stops_oneline, tab_stops_twoline):
    ...
    # Pad label_order for layout if needed (in case less than 4)
