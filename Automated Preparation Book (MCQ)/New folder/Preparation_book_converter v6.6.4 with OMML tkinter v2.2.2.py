    ...
    for node in tree.iterchildren():
        if node.tag.endswith('r'):
            ...
            if first_text_run and strip_label_prefix:
                # More aggressive stripping of label prefixes
                regex = re.compile(r'^\s*[\(\[]?\s*%s\s*[\.\)\]\।]?\s*' % re.escape(strip_label_prefix))
...
def parse_option(line):
    # Match formats like: "ক)", "(ক)", "ক.", "ক", etc.
    m = re.match(r'^\s*[\(\[]?\s*([কখগঘa-dA-D])[\.\)\]\।]?\s*(.*)', line)
    if m:
        # Return clean label and content
...
def format_mcq(mcq, doc, tab_stops_oneline, tab_stops_twoline):
    ...
    for i, (para, text) in enumerate(mcq['question_meta']):
        if i == 0:
            # Use only the text, skip label/serial re-insertion
    ...
    # Option rendering logic (one line, two line, or multiline)
    if option_layout == "oneline":
        ...
        for label in label_order[:4]:
            ...
            if not opt_tuple:
                continue
            _, opt_text = opt_tuple
            if col > 0:
                option_para.add_run('\t')
            # --- Always output label as: ক. text ---
            run = option_para.add_run(f"{label}. ")
            set_bangla_font(run, bold=True)
            # OMML/equation handling:
            omml_parts = [p for p in split_text_and_omml(opt_tuple[0]) if p[0] == "omml"]
            run2 = option_para.add_run(opt_text)
            set_bangla_font(run2)
            for ptype, pval in omml_parts:
                render_parts_to_para(option_para, [(ptype, pval)])
            col += 1
        ...
        for idx in range(4):
            ...
            set_bangla_font(run, bold=True)
            omml_parts = [p for p in split_text_and_omml(opt_tuple[0]) if p[0] == "omml"]
            run2 = p.add_run(opt_text)
            set_bangla_font(run2)
            for ptype, pval in omml_parts:
                render_parts_to_para(p, [(ptype, pval)])
    ...
    if ans_label or ans_text:
        ...
        if ans_label:
            ...
            if ans_tuple:
                _, ans_opt_text = ans_tuple
                run2 = p.add_run(ans_opt_text)
                set_bangla_font(run2)
                omml_parts = [pt for pt in split_text_and_omml(ans_tuple[0]) if pt[0] == "omml"]
                for ptype, pval in omml_parts:
                    render_parts_to_para(p, [(ptype, pval)])
            elif ans_text:
                p.add_run(ans_text)
    ...
    if mcq.get('explanation'):
        ...
        run2 = ep.add_run(exp_text)
        set_bangla_font(run2)
        omml_parts = [pt for pt in split_text_and_omml(para) if pt[0] == "omml"]
        for ptype, pval in omml_parts:
            render_parts_to_para(ep, [(ptype, pval)])
