...
LONG_OPTION_CHAR_LIMIT = 12  # Character length to trigger single-column layout
...
def format_mcq(mcq, doc, tab_stops):
    ...
    # 2. Options
    if _all_options_short(mcq['options_meta']):
        # One line for all four options, each starts at a fixed position using tab stops
        option_para = doc.add_paragraph()
        option_para.paragraph_format.tab_stops.clear_all()
        # Set tabs for ক, খ, গ, ঘ columns
        ...
        # Two lines (2+2), each with fixed tab stops for "ক"/"খ" and "গ"/"ঘ"
        for row_idx, row in enumerate([(0, 1), (2, 3)]):
            option_para = doc.add_paragraph()
            option_para.paragraph_format.tab_stops.clear_all()
            # Set tabs for two columns
            option_para.paragraph_format.tab_stops.add_tab_stop(tab_stops[0])
            option_para.paragraph_format.tab_stops.add_tab_stop(tab_stops[1])
            col = 0
            for idx in row:
...
def main():
    ...
    tab_stops = [Inches(0.8), Inches(1.6), Inches(2.4)]  # For "ক" at 0", "খ" at 2", "গ" at 4", "ঘ" at 6"
    # For two-column (2 per line): "ক"/"খ" at 0", 3.5"; "গ"/"ঘ" at 0", 3.5"
    two_col_tab_stops = [Inches(1.6), Inches(1.6)]

    mcqs = extract_mcqs(doc.paragraphs)
    print(f"Found and parsed {len(mcqs)} MCQs.")
    for mcq in mcqs:
        try:
            # Use different tab stops depending on layout
            if _all_options_short(mcq['options_meta']):
                format_mcq(mcq, outdoc, tab_stops)
            else:
                format_mcq(mcq, outdoc, two_col_tab_stops)
        except Exception as e:
