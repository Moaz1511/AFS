...
def format_mcq(mcq, doc):
    ...
    # Don't add serial number again if already present
    q_para = doc.add_paragraph()
    run = q_para.add_run(f"{mcq['serial']}. {qline0}")
    run.font.name = "Tiro Bangla"
    run.element.rPr.rFonts.set(qn('w:eastAsia'), 'Tiro Bangla')
    run.font.size = Pt(11)
    copy_paragraph_with_equations_and_style(src_para0, q_para)
    ...
    for idx in [0, 1]:
        ...
        # Don't add label inside opt_text anymore!
        if src_para:
            copy_paragraph_with_equations_and_style(src_para, opt_line1)
        else:
    ...
    for idx in [2, 3]:
        ...
        if src_para:
            copy_paragraph_with_equations_and_style(src_para, opt_line2)
        else:
    ...
    # Option prefix (e.g., " খ. ")
    run2 = ans_para.add_run(f" {ans}. ")
    run2.font.name = 'Tiro Bangla'
    run2.element.rPr.rFonts.set(qn('w:eastAsia'), 'Tiro Bangla')
    run2.font.size = Pt(11)

    # Add answer text/equation (with coloring if it's just text)
    ans_label = mcq.get('answer')
    answer_option = mcq['options'].get(ans_label, ('', None))
    ans_opt_text, ans_src_para = answer_option

    if ans_src_para is not None:
        # Inline: parse OMML/text for answer line (same as options code)
        from lxml import etree
        para_xml = etree.tostring(ans_src_para._element, encoding='unicode')
        tree = etree.fromstring(para_xml.encode('utf-8'))
        ns = {'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main',
              'm': 'http://schemas.openxmlformats.org/officeDocument/2006/math'}
        for node in tree.iterchildren():
            if node.tag.endswith('r'):
                texts = node.findall('.//w:t', namespaces=ns)
                for t in texts:
                    ans_run = ans_para.add_run(t.text)
                    ans_run.font.name = "Tiro Bangla"
                    ans_run.element.rPr.rFonts.set(qn('w:eastAsia'), "Tiro Bangla")
                    ans_run.font.size = Pt(11)
                    ans_run.font.color.rgb = RGBColor(0x08, 0x85, 0x65)  # always green for answer text!
                ...
                ans_para._p.append(omml_run)
    else:
        # Fallback: just add colored text
        ans_run = ans_para.add_run(ans_opt_text)
        ans_run.font.name = "Tiro Bangla"
        ans_run.element.rPr.rFonts.set(qn('w:eastAsia'), "Tiro Bangla")
        ans_run.font.size = Pt(11)
        ans_run.font.color.rgb = RGBColor(0x08, 0x85, 0x65)
        ans_run.font.bold = True
    ...
    for para in paragraphs:
        ...
        if state == 2:
            ...
            if m:
                label = m.group(1)
                opt_text = m.group(2)
                # Remove leading label (e.g., "ক. ") from opt_text if present
                if opt_text.startswith(label + "."):
                    opt_text = opt_text[len(label)+1:].lstrip()
                cur['options'][label] = (opt_text, para)
                continue
            elif ans_re.match(text):
                m = ans_re.match(text)
                cur['answer'] = m.group(1)
                ans_value = cur['options'].get(cur['answer'], ('', ''))[0]
                cur['answer_text'] = ans_value
                mcqs.append(cur)
                state = 0
            else:
                if cur['options']:
                    last_opt = list(cur['options'])[-1]
                    val, p = cur['options'][last_opt]
