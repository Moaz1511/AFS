...
def format_mcq(mcq, doc, tab_stops_oneline, tab_stops_twoline):
    ...
    if option_layout == "oneline":
        ...
        for label in label_order[:4]:
            ...
            # Use original split_text_and_omml but with more aggressive label stripping
            parts = split_text_and_omml(opt_tuple[0], strip_label_prefix=label)
            # Filter out any remaining label fragments
            filtered_parts = []
            for ptype, pval in parts:
                if ptype == "text":
                    # Remove any residual label fragments
                    pval = re.sub(r'^\s*[\(\[]?\s*%s\s*[\.\)\]\।]?\s*' % re.escape(label), '', pval)
                if pval.strip():
                    filtered_parts.append((ptype, pval))
            render_parts_to_para(option_para, filtered_parts)
            col += 1
        ...
        for idx in range(4):
            ...
            for ptype, pval in parts:
                ...
                if pval.strip():
                    filtered_parts.append((ptype, pval))
            render_parts_to_para(p, filtered_parts)
    ...
    if ans_label or ans_text:
        ...
        if ans_label:
            ...
            if ans_tuple:
                ...
                for ptype, pval in parts:
                    if ptype == "text":
                        pval = re.sub(r'^\s*[\(\[]?\s*%s\s*[\.\)\]\।]?\s*' % re.escape(ans_label), '', pval)
                    if pval.strip():
                        filtered_parts.append((ptype, pval))
                render_parts_to_para(p, filtered_parts)
            elif ans_text:
